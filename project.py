...
def newMenuItem(restaurant_id):
    ...
    if request.method == 'POST':
        newItem = MenuItem(name=request.form['name'], description=request.form['description'],
            price=request.form['price'], course=request.form['course'], restaurant_id=restaurant_id,user = user)
        session.add(newItem)
        session.commit()
...
def editMenuItem(restaurant_id, menu_id):
    ...
    if login_session['user_id'] != editedItem.user_id:
        flash("You are not allowed to edit  items to this menu. Create your own menuitem in order to edit .")
        return redirect(url_for('showMenu', restaurant_id=restaurant_id))
    if request.method == 'POST':
...
def deleteMenuItem(restaurant_id, menu_id):
    ...
    if login_session['user_id'] != itemToDelete.user_id:
        flash("You are not allowed to delete items to this restaurant. Create your own menuitem in order to delete .")
        return redirect(url_for('showMenu', restaurant_id=restaurant_id))
    if request.method == 'POST':
